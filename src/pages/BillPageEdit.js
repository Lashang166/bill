import React, { useState, useEffect, Fragment, useContext } from "react";
// import { Redirect } from "react-router-dom";
//import axios from "axios";
import { Helmet } from "react-helmet";
import BillDisplay from "../components/BillDisplayEdit";
import { BillInfoContext } from '../contexts/billInfo/billInfo.context';
import { getbill1, update1 } from "../API";
// const apiServer = "http://localhost:8080"; // "."
//const apiServer = '.'
//const apiServer = "https://bill.i-sabuy.xyz/Bill";


export default function BillPageEdit(props) {
  const [hasData, setHasData] = useState(false);
  const [billData, setBillData] = useState({});
  const [billName, setBillName] = useState('');
  const { billId } = props.match.params;

  const [COD, setCOD] = useState(-1);
  const { addNewBillInfo } = useContext(BillInfoContext);
  

  useEffect(() => {
    //axios.get(`${apiServer}/ci-api/bills/getBill/${billId}`)
    getbill1(billId)
      .then(result => {
        //console.log("axios get", JSON.stringify(result.data[0]));
        if (result.data.length > 0) {
          result.data[0].Data = JSON.parse(result.data[0].Data);

          const newResult = result.data[0].Data.items.filter((item, idx) => {
            if (item) {
              return item;
            }
          });
          result.data[0].Data.items = newResult;  
          setCOD(result.data[0].Data.items.COD);
          //console.log(result.data[0].Data.items.COD);
        }
        setBillData(result.data[0]);
        setHasData(true);
      })
      .catch(e => {
        //console.log("error", e.toString());
        setHasData(false);
        setBillData([]);
      });
  }, []);

  

const SaveBillData = billData => {

  const postData = { ...billData, Data: JSON.stringify(billData.Data) };

  
    //axios.post(`${apiServer}/ci-api/bills/update`, postData)
    update1(postData)
    .then(result => {
      //console.log("Saved");
    })
    .catch(e => {
      //console.log("Error updating", e);
      alert("Save error please try again");
      // window.location.reload();
    });
  
};
  
  const onClickSave = (inputName) => {

    //setBillData({...billData, billName});

    //console.log();
    if(inputName == ''){
      //alert("Please Input Name");
      return false;
    }
    else{
      addNewBillInfo({billName : inputName, billData});
      return true;
    }
    //addNewBillInfo(billData);
  }

  
  //console.log("billData", billData);
  return (
    <Fragment>
      <Helmet title={`Bill #${billId}`} />
      {/* {BillDisplay(hasData || billData.length > 0 ? billData[0] : null)} */}
      {hasData ? (
        <BillDisplay
          COD={COD}
          setCOD={setCOD}
          billData={billData}
          updateBillData={b => {

            
            setBillData({ ...b });
            SaveBillData(b);
          }}
          
          actionType="edit"
          onClickSaveFavorite={onClickSave}
        />
      ) : null}
      
    </Fragment>
  );
}
