...
const SidebarCategory = ({ deviceType: { mobile, tablet, desktop }, type, }) => {
    ...
    return (<CategoryWrapper>
      {(mobile || tablet) && (<Popover handler={<PopoverHandler>
              <div>
                <CategoryIcon />
            ...
            </PopoverHandler>} className="category-popover" content={<>
              <CategoryDropdown items={data.categories} iconList={iconTypes} handleCategorySelection={(slug) => handleCategorySelection(slug)} selectedQueries={selectedQueries}/>
            </>}/>)}
      {desktop && (<>
          <PopoverWrapper>
            <Popover handler={<PopoverHandler>
                  <div>
                    <CategoryIcon />
                    Select your Category
                  </div>
          ...
          <SidebarWrapper>
            <Sticky enabled={isSidebarSticky} top={110}>
              <Scrollbars universal autoHide autoHeight autoHeightMax={688}>
                <CategoryDropdown items={data.categories} iconList={iconTypes} handleCategorySelection={(id) => handleCategorySelection(id)} selectedQueries={selectedQueries}/>
              </Scrollbars>
            </Sticky>
          </SidebarWrapper>
        </>)}
