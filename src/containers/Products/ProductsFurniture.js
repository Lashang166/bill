...
export const Products = ({ deviceType, type, fetchLimit = 8, loadMore = true, }) => {
    ...
    return (<>
      <ProductsRow>
        {data.products.items.map((item, index) => (<ProductsCol key={index}>
            <ProductCardWrapper>
              <Fade duration={800} delay={index * 10} style={{ height: '100%' }}>
                <ProductCard title={item.title} image={item.gallery[0].url} discountInPercent={item.discountInPercent} onClick={() => handleQuickViewModal(item, deviceType, handleModalClose)}/>
              </Fade>
            </ProductCardWrapper>
          </ProductsCol>))}
      </ProductsRow>
