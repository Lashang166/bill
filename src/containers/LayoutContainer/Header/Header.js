...
const Header = ({ style, className, token, pathname, }) => {
    ...
    const { authState: { isAuthenticated }, authDispatch, } = useContext(AuthContext);
    ...
    const NavItem = (item) => {
        return (<NavLink key={item.link} onClick={() => resetSearch(item)} className='menu-item' href={item.link} label={item.label} icon={item.icon} iconClass='menu-item-icon'/>);
    };
    ...
    const isHomePage = pathname === HOME_PAGE ||
        ...
        pathname === BAGS_PAGE;
    return (<HeaderWrapper style={style} className={className}>
      <HeaderLeftSide>
        ...
        <MainMenu>
          <Popover className='right' handler={<SelectedType>
                <span>
              ...
              </SelectedType>} content={<>{MenuArray.map(NavItem)}</>}/>
        </MainMenu>
      </HeaderLeftSide>
      {isHomePage ? (<SearchBox className='headerSearch' handleSearch={(value) => handleSearch(value)} onClick={handleClickSearchButton} placeholder='Search anything...' hideType={true} minimal={true} showSvg={true} style={{ width: '100%' }} value={text || ''}/>) : null}
      <HeaderRightSide>
        <NavLink className='menu-item' href={OFFER_PAGE} label='Offer' intlId='navlinkOffer'/>
        <NavLink className='menu-item' href={HELP_PAGE} label='Need Help' intlId='navlinkHelp' iconClass='menu-icon' icon={<HelpIcon />}/>
        <LangSwithcer>
          <Popover className='right' handler={<SelectedLang>
                <Flag>{LanguageArray[selectedLangindex].icon}</Flag>
                <span>
                  <FormattedMessage id={LanguageArray[selectedLangindex].intlLangName} defaultMessage={LanguageArray[selectedLangindex].label}/>
                </span>
              </SelectedLang>} content={<>{LanguageArray.map(LanguageMenu)}</>}/>
        </LangSwithcer>

        {!isAuthenticated ? (<Button onClick={signInOutForm} size='small' title='Join' style={{ fontSize: 15, color: '#fff' }} intlButtonId='joinButton'/>) : (<Popover direction='right' className='user-pages-dropdown' handler={<img src={UserImage} alt='user'/>} content={<>
                {DropDownMenuArray.map((item, idx) => (<NavLink key={idx} className='menu-item' href={item.link} label={item.label} intlId={item.intlId}/>))}
